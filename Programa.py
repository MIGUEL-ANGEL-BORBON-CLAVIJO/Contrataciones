...
# FUNCIONES DE LOS BOTONEaS
        ...
        def conectar():
        # Aquí puedes agregar la lógica de conexión que necesites
                conectado = True  # Cambia esto según tu lógica de conexión

                if conectado:
                        messagebox.showinfo("Estado de Conexión", "Conectado con éxito")
                else:
                        messagebox.showerror("Estado de Conexión", "Error de conexión")
        ...
        def Agregar():
                try:
                  ...
                  if any([campo.get() == "" for campo in [ID, Nombre, Apellido, Cedula, CiudadExpedicion, Direccion, Contrato,
                                                ARL, EPS, FechaNacimiento, genero, Rh, Celular, Telefono, Correo,
                                                Cargo, FechaDocumento, Dependencia, Jefe, DescripcionContrato,
                                                ValorContrato, AutorizacionContrato, PAA, Banco, TipoCuenta,
                                                NumeroCuenta, CDP, FechaCDP, CRP]]):
                        messagebox.showerror("Error", "Por favor ingresa la información completa.")
                  else:
            # Conexión a la base de datos
                        base = pymysql.connect(host=server.get(), user=UserServer.get(), password=passServer.get(),
                                   database=baseServer.get())
                        cursor = base.cursor()

                        # Consulta de inserción con parámetros
                        sql = """INSERT INTO clientes (ID, Nombre, Apellido, Cedula, CiudadExpedicion, Direccion, Contrato, ARL, EPS, 
                                FechaNacimiento, genero, Rh, Celular, Telefono, Correo, Cargo, FechaDocumento, Dependencia, Jefe, 
                                DescripcionContrato, ValorContrato, AutorizacionContrato, PAA, Banco, TipoCuenta, NumeroCuenta, 
                                FechaCDP, CRP, CDP) 
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 
                                        %s, %s, %s, %s, %s)"""
                        
                        # Ejecución de la consulta
                        cursor.execute(sql, (ID.get(), Nombre.get(), Apellido.get(), Cedula.get(), CiudadExpedicion.get(),
                                                Direccion.get(), Contrato.get(), ARL.get(), EPS.get(), FechaNacimiento.get(),
                                                genero.get(), Rh.get(), Celular.get(), Telefono.get(), Correo.get(), Cargo.get(),
                                                FechaDocumento.get(), Dependencia.get(), Jefe.get(), DescripcionContrato.get(),
                                                ValorContrato.get(), AutorizacionContrato.get(), PAA.get(), Banco.get(),
                                                TipoCuenta.get(), NumeroCuenta.get(), FechaCDP.get(), CRP.get(), CDP.get()))

                        base.commit()  # Confirmar la transacción
                        base.close()   # Cerrar la conexión
...
# BARRA MENU
        ...
        menuconect.add_command(label="Conectar",activebackground ="green", command=conectar)
        menuconect.add_command(label="Salir",activebackground ="green",command=salir)
        ...
        bbddMenu.add_command(label="Limpiar",activebackground ="green",command=Limpiar)
        ...
        barraMenu.add_cascade(label="prueba...",activebackground ="orange")
        #FIN BARRA MENU
#Cajas de texto
